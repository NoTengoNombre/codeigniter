...
class Personas {
  ...
  public function modificar_personas() {
    ...
    if ($conn->connect_error) {
      die("Error : No se ha establecido la conexion . " . $conn->connect_error);
    }
    if (isset($_REQUEST['cod_persona']) && isset($_REQUEST['nombre']) && isset($_REQUEST['apellidos']) && isset($_REQUEST['pais'])) {
      $sql = "UPDATE personas SET nombre='" . $this->nombre . "' , apellidos='" . $this->apellidos . "' , pais='" . $this->pais . "' WHERE cod_persona='" . $this->cod_persona . "';";
