...
//    $this->load->view('main_template', $data); // pantilla para pasar datos del array
  ...
  public function comprobar_login() {
    ...
    $this->form_validation->set_rules("password", "Password", "required|min_length[5]"); // fijar reglas de validacion
    ...
    if ($this->form_validation->run() == FALSE) { // formulario no esta corriendo - envia login
      ...
      if ($this->login_model->comprobar_usuario($_REQUEST['email'], $_REQUEST['password'])) { // si son correctos
